...
type ConvItem = {
  ...
  listings: ListingPreview | null; buyer: OtherUser; seller: OtherUser
  last_message: MessagePreview | null; unread_count: number
}
      ...
      {conversations.length === 0 ? (
        ...
        <div className="bg-white rounded-xl border border-[#f0e8d8] divide-y divide-gray-100">
          {conversations.map(conv => {
            ...
            return (
              <Link key={conv.id} href={`/messages/conversation?id=${conv.id}`}
                ...
                <div className="relative flex-shrink-0">
                  ...
                  {hasUnread && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-[#722f37] rounded-full border-2 border-white" />}
                </div>
